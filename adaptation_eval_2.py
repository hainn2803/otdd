...
if method == "sotdd":
    display_method = f"s-OTDD ({num_moments} moments - {num_proj} projections)"
...
if method == "sotdd":
    ...
    dist_path = f"saved_nist/dist/sotdd_dist_30_03_2025_noclamp_proj{num_proj}_{proj_type}_5moment.json"
...
for each_run in os.listdir(saved_dir):
    ...
    if "nist" in each_run:
        ...
        for dt_name in os.listdir(baseline_weight_path):
            ...
            each_run_acc_baseline[dt_name] = dict()
...
plt.ylabel(f"Performance Gap in {metric.capitalize()} (%)", fontsize=FONT_SIZE - 2)
...
if method == "sotdd":
    plt.savefig(f'{saved_dir}/rebuttal_{method}_distance_num_moments_{num_moments}_{proj_type}_proj{num_proj}_{metric}.png')
    plt.savefig(f'{saved_dir}/rebuttal_{method}_distance_num_moments_{num_moments}_{proj_type}_proj{num_proj}_{metric}.pdf')
else:
