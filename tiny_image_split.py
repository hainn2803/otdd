...
def random_split_into_tasks(
    ...
    # 1) Identify all unique labels from the training set (or union with test set, if you prefer)
    all_labels_train = torch.tensor(trainset.targets)
    unique_labels = torch.unique(all_labels_train).tolist()
    # 2) Shuffle those labels in-place
    random.shuffle(unique_labels)

    # 3) Create a list of label-groups, each of size `labels_per_task`
    label_groups = []
    ...
    # Convert train/test targets to tensor for indexing
    ...
    # 4) For each label group, find indices in train and test sets
...
def main():
    ...
    for task_id in range(num_tasks):
        save_dataset(dataloader=train_loaders[task_id], filepath=f"saved_split_task_10/data/trainset_{task_id}.pt")
        save_dataset(dataloader=test_loaders[task_id], filepath=f"saved_split_task_10/data/test_{task_id}.pt")
